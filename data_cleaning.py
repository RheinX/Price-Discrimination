...
def clean_auction():
    ...
    for v, d in item_ordered:
        item_file.write(
            str(item[v]['id']) + '\t' + str(item[v]['time']) + '\t' + str(item[v]['price']) + '\t' + item[v][
                'name'] + '\n')
    item_file.close()
...
def clean_set(read_file_name, write_file_prefix, category_m=None, user_m=None, control=False):
    """
    ...
    if control:
        category_mapping = category_m
        user_mapping = user_m

    with open(file_prefix + read_file_name) as f:
        f_csv = csv.DictReader(f)
        for row in f_csv:
            ...
            # ignore him if pid represent new user when read test data
            if pid not in user_mapping:
                continue
...
if __name__ == '__main__':
    # clean_auction()
    train_read_file_name = "TrainingSet.csv"
    test_read_file_name = "TestSet.csv"
    train_write_file_prefix = "clean_data_2/"
